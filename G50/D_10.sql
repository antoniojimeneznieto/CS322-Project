WITH VALID_REVIEWS AS (SELECT Business_Id, COUNT(*) Count FROM Reviews WHERE User_Id IN
    ...
    (SELECT City_Name, SUM(COUNT) SUM FROM VALID_REVIEWS VR INNER JOIN
        (SELECT Business_Id, City_Name FROM Business_Location) BL
        ON BL.Business_Id=VR.Business_Id GROUP BY City_Name) CC
    ON C.City_Name = CC.City_Name ORDER BY Review_Count, City_Name;