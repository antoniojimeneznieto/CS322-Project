-- Inefficient Query
CREATE INDEX idx_bhcat_busid ON BUSINESS_HAS_CATEGORIES(BUSINESS_ID);
CREATE INDEX idx_bpt_busid ON BUSINESS_PARKING_TYPE(BUSINESS_ID);
CREATE INDEX idx_bus_busid ON BUSINESS(BUSINESS_ID);

SELECT AVG(B.STARS), SUM(B.REVIEW_COUNT)
FROM BUSINESS B
WHERE (
    SELECT COUNT(*)
    FROM BUSINESS_HAS_CATEGORIES BHC
    WHERE B.BUSINESS_ID = BHC.BUSINESS_ID
) >= 2
AND (
    SELECT COUNT(*)
    FROM Business_parking_type BPT
    WHERE B.BUSINESS_ID = BPT.BUSINESS_ID
) > 1;

SELECT AVG(STARS) AS stars, SUM(REVIEW_COUNT) AS review_count
FROM BUSINESS
WHERE BUSINESS_ID IN (
    SELECT BHC.BUSINESS_ID
    FROM BUSINESS_HAS_CATEGORIES BHC
    GROUP BY BHC.BUSINESS_ID
    HAVING COUNT(CATEGORY_ID) >= 2
)
AND BUSINESS_ID IN (
    SELECT BPT.BUSINESS_ID
    FROM BUSINESS_PARKING_TYPE BPT
    GROUP BY BPT.BUSINESS_ID
    HAVING COUNT(PARKING_TYPE_ID) > 1
);

-- Optimized Query
CREATE INDEX idx_bhcat_busid ON BUSINESS_HAS_CATEGORIES(BUSINESS_ID);
CREATE INDEX idx_bpt_busid ON BUSINESS_PARKING_TYPE(BUSINESS_ID);
CREATE INDEX idx_bus_busid ON BUSINESS(BUSINESS_ID);

WITH QUERY_BLOCK1 AS (
  SELECT BUSINESS_ID
  FROM BUSINESS_HAS_CATEGORIES
  GROUP BY BUSINESS_ID
  HAVING COUNT(CATEGORY_ID) >= 2
),
QUERY_BLOCK2 AS (
  SELECT BUSINESS_ID
  FROM BUSINESS_PARKING_TYPE
  GROUP BY BUSINESS_ID
  HAVING COUNT(PARKING_TYPE_ID) > 1
),
COMBINED AS (
  SELECT QUERY_BLOCK1.BUSINESS_ID
  FROM QUERY_BLOCK1
  INNER JOIN QUERY_BLOCK2 ON QUERY_BLOCK1.BUSINESS_ID = QUERY_BLOCK2.BUSINESS_ID
)
SELECT AVG(stars), SUM(review_count)
FROM BUSINESS
WHERE BUSINESS_ID IN (SELECT BUSINESS_ID FROM COMBINED);
